...
function beginGame() {
    ...
    var cycleNum = 1;
    while ( mafiaAlive > 0 && (playersAlive - mafiaAlive) > 1 ) {
         if (cycleNum % 2 === 0) {
            dayCycle();
            console.log("day cycle complete");
            console.log('cycle ' + cycleNum/2);
        }
        else {
            nightCycle();
...
function assignCharacters() {
    ...
    //     [SOCKET_LIST[i], SOCKET_LIST[j]] = [SOCKET_LIST[j], SOCKET_LIST[i]];
      ...
      var rolesAvailable = [0,1,2,3];
      var j = -1;

        for (var i = 0; i < playerNum; i++){
          do{
            j = Math.floor(Math.random() * playerNum);
          }while (Player.list[j].self.role != -1);
          Player.list[j].self.role = rolesAvailable[i];
        }
    // 0 = mafia
    // 1 = doctor
    // 2 = dective
    ...
    for (var i in SOCKET_LIST) {
        ...
        SOCKET_LIST[i].emit(Player.list[i].self.role);
